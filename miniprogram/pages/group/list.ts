  ...
  async onShow() {
    ...
    const list = raw.map((item: any) => {
      ...
      return {
        ...
        minGroupPriceCent: toFiniteNumber(row.totalGroupPriceCent) || toFiniteNumber(row.minGroupPriceCent) || toFiniteNumber(row.group_price_cent) || 0,
        originTotalCent: toFiniteNumber(row.originTotalCent) || 0,
